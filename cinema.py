...
# ==================== 配置 ====================
WORKFLOW_PATH = "workflows/cinema_6angles.json"
TEMPLATES_DIR = "user_templates_cinema"
TEMP_DIR = "temp"
OUTPUT_DIR = "output"
...
# ==================== 默认参数 ====================
DEFAULT_NEGATIVE = "worst quality, low quality, blurry, noisy, pixelated, distorted, deformed, ugly, watermark, extra limbs, fused fingers, bad proportions, disfigured, out of frame, cropped, asymmetrical, tilted, crooked"

DEFAULT_PROMPTS = {
    "front": "cinematic still, movie scene, a person, Hollywood lighting, film grain, shallow depth of field, dramatic lighting, anamorphic lens, 35mm, professional actor portrait, blockbuster, front view, looking at camera, plain background",
    "back": "cinematic still, movie scene, a person, from behind, back view, looking away, Hollywood lighting, film grain, dramatic lighting, plain background",
    "side": "cinematic still, movie scene, a person, side view, profile shot, facing left, Hollywood lighting, film grain, dramatic lighting, plain background",
    "threequarter": "cinematic still, movie scene, a person, three quarter view, turned slightly, looking at camera, Hollywood lighting, film grain, dramatic lighting, plain background",
    "rightfront": "cinematic still, movie scene, a person, facing right, looking right, right side angle, Hollywood lighting, film grain, dramatic lighting, plain background",
    "top": "cinematic still, movie scene, a person, looking up, from below, low angle, looking upward, Hollywood lighting, film grain, dramatic lighting, plain background"
}
...
def save_images_to_project(images, prefix="cinema"):
    """保存单张图片和拼图到规范的文件夹结构"""
...
def run_comfyui_workflow(workflow, progress):
    ...
    debug_path = os.path.join(TEMP_DIR, f"cinema_api_send_{int(time.time())}.json")
...
def generate_images(
    ...
    debug_path = os.path.join(TEMP_DIR, f"cinema_debug_{int(time.time())}.json")
    with open(debug_path, 'w', encoding='utf-8') as f:
        json.dump(workflow, f, indent=2)
    ...
    if images and any(images):
        saved_paths, job_dir = save_images_to_project(images, prefix="cinema")
...
# ==================== UI 界面 ====================
def create_ui():
    with gr.Blocks(title="FaceOrbit - 影视明星模式", theme=gr.themes.Soft()) as demo:
        gr.Markdown("""
        # 🎬 FaceOrbit - 影视明星模式
        ### 电影质感人像 | 好莱坞级光影效果
        """)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Accordion("⚙️ 生成参数", open=False):
                    ip_weight = gr.Slider(0.5, 1.0, value=0.9, step=0.01, label="🎭 相似度")
                    cn_strength = gr.Slider(0.0, 0.8, value=0.5, step=0.01, label="🎯 姿态控制")
                    denoise = gr.Slider(0.5, 0.9, value=0.72, step=0.01, label="✨ 创意度")
                    cfg = gr.Slider(1.0, 10.0, value=2.6, step=0.1, label="📝 提示词引导")
        ...
        def set_params(params):
            ...
            return [
                ...
                params.get("ip_weight", 0.9), params.get("cn_strength", 0.5),
                params.get("denoise", 0.72), params.get("cfg", 2.6), params.get("steps", 30),
                params.get("correction_strength", 0.55), params.get("width", 896), params.get("height", 1152)
            ]
