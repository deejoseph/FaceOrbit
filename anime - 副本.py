...
TEMPLATES_DIR = "user_templates_anime"
TEMP_DIR = "temp"
OUTPUT_DIR = "output"
...
def run_comfyui_workflow(workflow, progress):
    ...
    debug_path = os.path.join(TEMP_DIR, f"anime_api_send_{int(time.time())}.json")
...
def generate_images(
    ...
    # 设置种子
    sampler_nodes = ["11_front", "11_back", "11_side", "11_threequarter", "11_rightfront", "11_top"]
    if use_fixed_seed:
    ...
    debug_path = os.path.join(TEMP_DIR, f"anime_debug_{int(time.time())}.json")
    with open(debug_path, 'w', encoding='utf-8') as f:
        json.dump(workflow, f, indent=2)
...
# ==================== UI 界面 ====================
def create_ui():
    with gr.Blocks(title="FaceOrbit - 二次元动漫模式", theme=gr.themes.Soft()) as demo:
        gr.Markdown("""
        # 🎨 FaceOrbit - 二次元动漫模式
        ### 上传照片，生成属于你的动漫角色形象（6个角度）
        
        > 💡 **提示**：本模式支持多角度生成，每次生成使用随机种子，可勾选固定种子复现结果。
        """)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown("> ✨ **照片不影响**：服装、背景、姿势、角度（由提示词控制）")
                ...
                with gr.Group(visible=True) as easy_mode_group:
                    ...
                    preset_character.change(
                        lambda c: f"**角色描述**：{PRESET_CHARACTERS.get(c, {}).get('description', '')}",
                        preset_character, char_desc
                    )
                ...
                with gr.Group(visible=False) as advanced_mode_group:
                    with gr.Tabs():
                        with gr.TabItem("正面"):
                            adv_pf = gr.Textbox(label="正向提示词", value=DEFAULT_PROMPTS["front"], lines=2)
                        with gr.TabItem("背面"):
                            adv_pb = gr.Textbox(label="正向提示词", value=DEFAULT_PROMPTS["back"], lines=2)
                        with gr.TabItem("侧面"):
                            adv_ps = gr.Textbox(label="正向提示词", value=DEFAULT_PROMPTS["side"], lines=2)
                        with gr.TabItem("四分之三"):
                            adv_pt = gr.Textbox(label="正向提示词", value=DEFAULT_PROMPTS["threequarter"], lines=2)
                        with gr.TabItem("右前"):
                            adv_pr = gr.Textbox(label="正向提示词", value=DEFAULT_PROMPTS["rightfront"], lines=2)
                        with gr.TabItem("顶视"):
                            adv_po = gr.Textbox(label="正向提示词", value=DEFAULT_PROMPTS["top"], lines=2)
        ...
        def get_params():
            if mode_selector.value == "🎮 体验模式 - 快速体验预设角色":
                ...
                return {
                    ...
                    "prompt_front": adv_pf.value, "prompt_back": adv_pb.value,
                    "prompt_side": adv_ps.value, "prompt_threequarter": adv_pt.value,
                    "prompt_rightfront": adv_pr.value, "prompt_top": adv_po.value,
                    ...
                    "correction_strength": corr_str.value,
...
if __name__ == "__main__":
    ...
    print(f"🚀 正在启动 FaceOrbit - 二次元动漫模式...")
