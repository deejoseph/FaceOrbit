...
TEMPLATES_DIR = "user_templates_ghostmix"
TEMP_DIR = "temp"
OUTPUT_DIR = "output"
...
def run_comfyui_workflow(workflow, progress):
    ...
    debug_path = os.path.join(TEMP_DIR, f"ghostmix_api_send_{int(time.time())}.json")
...
def generate_images(
    ...
    debug_path = os.path.join(TEMP_DIR, f"ghostmix_debug_{int(time.time())}.json")
    with open(debug_path, 'w', encoding='utf-8') as f:
        json.dump(workflow, f, indent=2)
...
# ==================== UI 界面 ====================
def create_ui():
    with gr.Blocks(title="FaceOrbit - 立体感动漫模式", theme=gr.themes.Soft()) as demo:
        gr.Markdown("""
        # ✨ FaceOrbit - 立体感动漫模式
        ### 2.5D 游戏CG风格 | 生成立体感十足的动漫角色
        """)
...
if __name__ == "__main__":
    demo = create_ui()
    local_url = "http://127.0.0.1:7863"
    
    print(f"🚀 正在启动 FaceOrbit - 立体感动漫模式...")
